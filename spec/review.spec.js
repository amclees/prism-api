      ...
      nodes: nodesObject
        ...
        .then((saved) => {
          ...
          Review.findById(testId).then((testReview) => {
            expect(testReview.nodes[id1].finishDate).toEqual(new Date('2200-02-09'));
            expect(testReview.nodes[id2].finishDate).toEqual(new Date('2200-02-16'));
            expect(testReview.nodes[id3].finishDate).toEqual(new Date('2200-02-28'));
            done();
          });
        });
  });
